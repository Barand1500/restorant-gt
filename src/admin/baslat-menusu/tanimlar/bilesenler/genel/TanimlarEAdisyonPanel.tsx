...
interface TanimlarEAdisyonPanelProps {
  form: TanimlarGenelForm;
  onFormDegistir: (form: TanimlarGenelForm) => void;
}

export function TanimlarEAdisyonPanel({ form, onFormDegistir }: TanimlarEAdisyonPanelProps) {
  return (
    <section className="ap-tanimlar-inline-panel" aria-label="E Adisyon tasarımı — yazdır">
      <div className="ap-tanimlar-inline-panel-baslik">
        <div>
          <h3 className="ap-heading text-sm font-semibold">Print</h3>
          <p className="ap-muted mt-0.5 text-xs">E Adisyon yazıcı ve tasarım seçimi</p>
        </div>
      </div>
      ...
      <div className="ap-tanimlar-print-grid">
        <fieldset className="ap-tanimlar-print-alan">
          ...
          <FormAcilirSecim
            aria-label="E Adisyon yazıcısı"
            value={form.eAdisyonYazicisi}
            onChange={(eAdisyonYazicisi) => onFormDegistir({ ...form, eAdisyonYazicisi })}
        ...
        <fieldset className="ap-tanimlar-print-alan">
          ...
          <select
            ...
            value={form.eAdisyonTasarim}
            onChange={(e) => onFormDegistir({ ...form, eAdisyonTasarim: e.target.value })}
          >
